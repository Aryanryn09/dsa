...
//thsts the only possible logic behind
           ...
           if(arr[s]==arr[mid]&&arr[e]==arr[mid ]){
               //if element at start and end are equal the start++ and end --
               if( arr[s]>arr[s+1]){
                   return s;
               }
               s++;
               if(arr[e]<arr[e-1]){
                   return e;//check if the s is pivit
               }
               e--;
           }
           //it may also be true that ele at start  may be the pivit in that case print
              ...
              e=mid -1;
              //paisa fr+++++++  ++-------i dont believe in voi;lem=nce do you
...
}//if elements are duplicate they are skipped
